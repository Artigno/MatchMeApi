...
The insight that makes this worth building now: automated image analysis can extract brand, color, category, and preliminary size from a photo in seconds — data extraction that was too expensive or inaccurate three years ago is now commodity. And no tool currently cross-posts a single listing to Vinted + OLX + Facebook Marketplace at once; that integration gap is unoccupied.
...
**Secondary persona** (not MVP priority): active high-volume resellers who list 20+ items per week and need bulk automation — out of MVP scope.
...
MVP scope: the app generates the listing card and the user posts manually (copy to clipboard / open platform in browser). Platform API posting is V2.
...
### Guardrails
1. If automated analysis cannot confidently determine brand, color, or category from a photo, it shows an empty field for the user to fill — never shows a confidently wrong value. Trust over automation.
...
- "Copy to clipboard" and "Open platform" actions are both available on the completed card
...
- FR-006: User can copy a completed listing card to clipboard or open a resale platform in the browser with listing data pre-populated. Priority: must-have
  > Socrates: Counter-argument considered: none — clipboard/browser-open is MVP-appropriate given the deliberate decision to avoid platform API integration in v1. FR stands.

### Account & Sync
- FR-007: User can create an account or sign in to sync wardrobe to the cloud. Priority: must-have
...
- FR-011: User can describe their emotional state or occasion in a sentence and receive an outfit suggestion from their wardrobe. Priority: nice-to-have
  > Socrates: Counter-argument considered: "this is the feature that differentiates from every other wardrobe app — no one does mood → outfit well." Resolution: differentiator framing confirmed; execution quality (suggestion relevance) is the primary risk. Nice-to-have stands.

- FR-012: User can visualize how an outfit looks on a virtual avatar. Priority: nice-to-have
  > Socrates: Counter-argument considered: "avatar body shape won't match the user's — visualization misleads more than it helps." Resolution: kept as nice-to-have; poor fit visualization erodes trust faster than no visualization. High visual accuracy is a hard gate before shipping this feature.
...
- FR-013: User can receive proactive shopping suggestions based on wardrobe gaps and promotions matching their size. Priority: nice-to-have
  > Socrates: Counter-argument considered: "proactive suggestions that aren't relevant feel like spam — users disable notifications immediately." Resolution: kept; suggestion quality must precede volume. Quality-before-quantity is a hard constraint on this feature's rollout.
...
- The automated classification never outputs a value in any field when its confidence is below an internal threshold; empty fields are preferable to plausible-but-wrong values. A user who sees a populated field can trust it.
...
**Recommendation rule** (outfit flow): The app takes user-provided context — current weather conditions, a mood or occasion described in a sentence or selected from presented options — as its input and outputs an outfit selection from the items currently in the user's wardrobe catalogue. The user encounters this rule when they open the "What should I wear?" flow. The rule has no value until the user has catalogued garments; cold-start (empty wardrobe) is a known limitation.

The wardrobe catalogue is the shared asset both rules operate on. The classification rule can run without a stored catalogue (it produces a listing card the user exports immediately); the recommendation rule requires a non-empty catalogue and will not offer suggestions when the wardrobe is empty.

## Access Control

Two-state model: **guest → account**.

- **Guest (unauthenticated)**: user can open the app and use core wardrobe features (add/browse garments) without creating an account. Data stored on-device only; no server contact.
- **Account (email + password or social login)**: user creates or signs in. Wardrobe data syncs across the user's devices.

Flat single-user model — no roles, no sharing, no admin panel in MVP. One account = one wardrobe. Connecting external resale platform accounts for automated posting is a V2 capability, not part of MVP.

## Non-Goals

- **No direct API posting to Vinted, OLX, or Facebook Marketplace in MVP.** The app generates a ready-to-paste listing card; the user posts manually. Platform posting automation is V2.
- **No social features.** Single-user experience only. No public profiles, outfit sharing, community feed, likes, or social graph.
- **No bulk import from camera roll.** One garment at a time. Batch classification of an existing wardrobe from the photo library is V2.
- **No real-time resale price lookup.** The listing card suggests a description and condition; it does not fetch live market prices from any platform.

## Open Questions

1. **Cold-start onboarding for outfit suggestions (FR-010).** How does the app incentivize new users to catalogue enough garments before outfit suggestions surface? If the wardrobe is empty, the recommendation rule has no value. Owner: design/user. Block: no (FR-010 is nice-to-have; must resolve before shipping that feature).

2. **Delete vs soft-archive for garment removal (FR-005).** The Socratic round flagged that soft-archive (hide but keep) is safer than hard delete for accidental removals. The product-level FR stands either way, but the implementation approach should be decided before building. Owner: user. Block: no.

3. **`qps` and `data_volume` in target_scale.** Emitted as `low` and `small` respectively based on "just me, or a handful" user scale. These are ballpark estimates. Revisit if the user scale changes. Owner: user. Block: no.
